...
def scan(agent_url: str, timeout: int, verbose: bool):
    ...
    try:
        ...
        if trust_score_100 < 50:
            click.echo(f"\n⚠️  Recommendations:")
            click.echo(f"   • This agent has a low trust score")
            click.echo(f"   • Use X-User-Override: true header to proceed")
            click.echo(f"   • Avoid sending sensitive data")
            click.echo(f"   • Monitor quarantine logs")
        ...
        click.echo(f"   Agent may be down or unreachable", err=True)
        sys.exit(1)
    except httpx.HTTPStatusError as e:
        click.echo(f"\n❌ HTTP {e.response.status_code}: {e.response.reason_phrase}", err=True)
        if e.response.status_code == 404:
            click.echo(f"   Manifest endpoint not found", err=True)
